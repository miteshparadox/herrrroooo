...
async def attack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    url = context.args[0]
    await update.message.reply_text(
        f"<blockquote>Attack request received for: <b>{url}</b>\nYour credits before attack: <b>{user_credits['credits']}</b>\n</blockquote>",
            if not is_admin:
            await context.bot.send_message(
                chat_id=8158960738,
    ...
    try:
        ...
        success_text = (
            ...
            f"Credits left: <b>{user_credits['credits']}</b>\n"
            f"</blockquote>"
            if not is_admin:
                await context.bot.send_message(
                    chat_id=8158960738,
        ...
        hulk_process = None
        await update.message.reply_text(f"Error: {e}\nPlease make sure Python is installed and hulk.py is in this folder.")
